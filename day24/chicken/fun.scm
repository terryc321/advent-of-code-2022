...
;; highly critical of imports and what we import to the scheme environment
...
(define (search puzzle)
  (bind (width height) (first puzzle)
	(letrec ((next-wind (lambda (wind)
			      (bind (x y dir) wind
				      (cond
				       ((eq? dir '>) (right x y))
				       ((eq? dir '<) (left x y))
...
;; for any specific blizzard know always in same direction 
(define (bliz width height x y direction time)
  
  ;; blizzard always moves right
  (define (right x y step) 
    (cond
     ((= step 0) (list x y))
     (#t (let ((x2 (+ x 1))(y2 y))
	   (when (> x2 width)
  ...
  ;; bliz action
  (cond
   ((eq? direction '>) (right x y time))
   ((eq? direction '<) (left x y time))
   ((eq? direction '^) (up x y time))
   ((eq? direction 'v) (down x y time))))
