...
(declaim (optimize (speed 1)(debug 3)(safety 3)(space 1)))
...
;; redefine ... is this ok ?
(defun convert-lines-to-grid (lines)
  (let ((x 0)(y 0)(max-x nil)(max-y nil)(hash (make-hash-table :test #'equalp)))
    (loop for line in lines do
      (incf y)
      (when (or (not max-y)(> y max-y)) (setq max-y y))
      (setq x 0)
    ...
    (setf (gethash 'max-x hash) max-x)
    (setf (gethash 'max-y hash) max-y)
    hash))
...
(defparameter *grid* (convert-lines-to-grid
